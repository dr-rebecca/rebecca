    ...
    class Breadcrumb_Trail
    {
        ...
        public function trail()
        {
            ...
            if ( 0 < $item_count ) {
                ...
                $breadcrumb .= '<ul class="trail-items breadcrumb">';
        ...
        {
            $defaults     = array(
                'browse'              => esc_html__('Browse:', 'armania'),
                'aria_label'          => esc_attr_x('Breadcrumbs', 'breadcrumbs aria label', 'armania'),
                'home'                => esc_html__('Home', 'armania'),
                'error_404'           => esc_html__('404 Not Found', 'armania'),
                'archives'            => esc_html__('Archives', 'armania'),
                // Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
                'search'              => esc_html__('Search results for &#8220;%s&#8221;', 'armania'),
                // Translators: %s is the page number.
                'paged'               => esc_html__('Page %s', 'armania'),
                // Translators: Minute archive title. %s is the minute time format.
                'archive_minute'      => esc_html__('Minute %s', 'armania'),
                // Translators: Weekly archive title. %s is the week date format.
                'archive_week'        => esc_html__('Week %s', 'armania'),
        ...
        protected function add_posts_page_items()
        {
            ...
            if ( is_paged() ) {
                $this->items[] = sprintf('<a rel="v:url" property="v:title" href="%s">%s</a>',
        ...
        protected function add_minute_hour_archive_items()
        {
            ...
            if ( true === $this->args['show_title'] ) {
                $this->items[] = sprintf($this->labels['archive_minute_hour'],
                    get_the_time(esc_html_x('g:i a', 'minute and hour archives time format', 'armania')));
            }
        }
        ...
        protected function add_minute_archive_items()
        {
            ...
            if ( true === $this->args['show_title'] ) {
                $this->items[] = sprintf($this->labels['archive_minute'],
                    get_the_time(esc_html_x('i', 'minute archives time format', 'armania')));
            }
        }
        ...
        protected function add_hour_archive_items()
        {
            ...
            if ( true === $this->args['show_title'] ) {
                $this->items[] = sprintf($this->labels['archive_hour'],
                    get_the_time(esc_html_x('g a', 'hour archives time format', 'armania')));
            }
        }
        ...
        protected function add_day_archive_items()
        {
            ...
            $year  = sprintf($this->labels['archive_year'],
                get_the_time(esc_html_x('Y', 'yearly archives date format', 'armania')));
            $month = sprintf($this->labels['archive_month'],
                get_the_time(esc_html_x('F', 'monthly archives date format', 'armania')));
            $day   = sprintf($this->labels['archive_day'],
                get_the_time(esc_html_x('j', 'daily archives date format', 'armania')));
        ...
        protected function add_week_archive_items()
        {
            ...
            $week = sprintf($this->labels['archive_week'],
                get_the_time(esc_html_x('W', 'weekly archives date format', 'armania')));
            // Add the year item.
            $this->items[] = sprintf('<a rel="v:url" property="v:title" href="%s">%s</a>',
                esc_url(get_year_link(get_the_time('Y'))), $year);
            // Add the week item.
        ...
        protected function add_post_parents( $post_id )
        {
            ...
            while ( $post_id ) {
                ...
                $parents[] = sprintf('<a rel="v:url" property="v:title" href="%s">%s</a>',

...
class Shortcode_Ovic_Network extends Ovic_Addon_Shortcode
{
        ...
        <div class="<?php echo esc_attr(implode(' ', $class_item)); ?>">
            <a href="<?php echo esc_url($image['url']) ?>"
               ...
               class="<?php echo esc_attr($class_wrap); ?>">
                <figure>
                    <img class="<?php echo esc_attr($class_img); ?>" src="<?php echo esc_attr($img_lazy); ?>"
                         data-src="<?php echo esc_url($image['src']); ?>"
                        <?php echo image_hwstring($image['width'], $image['height']); ?>
                         alt="<?php echo esc_attr($image['alt']); ?>"/>
                </figure>
                <?php if ( !empty($atts['text']) ): ?>
                    <span class="title"><?php echo esc_html( $atts['text'] ); ?></span>
                <?php endif; ?>
            </a>
        </div>
        <?php
    ...
    public function api_flickr($atts)
    {
        ...
        if (intval($id_flickr) === 0) {
            echo sprintf('<div class="alert alert-warning"><strong>%s</strong> %s</div>',
                ...
                esc_html__('No user ID specified.', 'armania')
            );
        }
        if (empty($flickr) || $flickr === false && $id_flickr) {
            ...
            if (!is_wp_error($response)) {
                ...
                if ($response_code != 200) {
                    echo sprintf('<div class="alert alert-warning"><strong>%s</strong> %s</div>',
                        esc_html__('Warning!', 'armania'),
                        esc_html__('User ID do not match. Please check again.', 'armania')
                    );
                } else {
    ...
    public function api_instagram($atts)
    {
        ...
        if (empty($instagram) || $instagram === false && $atts['id_instagram'] && $atts['token']) {
            ...
            $instagram_api = add_query_arg(
                ...
                "{$protocol}://api.instagram.com/v1/users/{$id_instagram}/media/recent"
            );
            ...
            if (!is_wp_error($response)) {
                ...
                if ($response_code != 200) {
                    echo sprintf('<div class="alert alert-warning"><strong>%s</strong> %s</div>',
                        esc_html__('Warning!', 'armania'),
                        esc_html__('User ID and access token do not match. Please check again.', 'armania')
                    );
                } else {
    ...
    public function content($atts, $content = null)
    {
        ...
        if ($atts['list_style'] != 'none') {
            $class_list[] = $atts['slides_rows_space'];

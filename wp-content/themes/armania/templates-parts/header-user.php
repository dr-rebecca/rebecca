...
$title = sprintf( esc_html__( 'Register %s or Sign in', 'armania' ), '<br>' );
if ( !empty( $text ) ) {
    $title = $text;
}
?>
<div class="block-userlink armania-dropdown">
	<?php if ( is_user_logged_in() ): ?>
        <a data-armania="armania-dropdown" class="woo-user-link"
		...
		<?php else: ?>
            <ul class="sub-menu">
                ...
                <li class="menu-item">
                    <a href="<?php echo esc_url( get_bloginfo( 'rss2_url' ) ); ?>">
						...
						echo sprintf(
							...
							esc_html__( 'Entries', 'armania' ),
							esc_html__( 'RSS', 'armania' )
						);
						?>
                    </a>
                </li>
				...
				echo sprintf(
					...
					esc_attr__( 'Powered by WordPress, state-of-the-art semantic personal publishing platform.', 'armania' ),
					esc_html_x( 'WordPress.org', 'meta widget link text', 'armania' )
				);
				wp_meta();
				?>
            </ul>

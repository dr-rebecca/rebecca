...
$post_style   = armania_get_option('blog_list_style', 'standard');
$sub_class    = 'blog-page';
if (is_single()) {
    $post_style = armania_get_option('single_layout', 'standard');
    $sub_class  = 'post-page';
}
$main_class   = array(
    "container",
...
$post_banner  = armania_get_option('blog_banner');
$style_banner = '';
if (!empty($post_banner) && $post_banner['url'] != '') {
    $style_banner = $post_banner['url'];
}
?>
    <!-- .site-content-contain -->
    <div id="content" class="<?php echo implode(' ', $main_class); ?>">

        <div class="page-head" style="background-image: url(<?php echo esc_url($style_banner); ?>);">
            <div class="container">
                <?php armania_page_title(); ?>
            </div>
        </div>
        ...
        <?php if ($page_layout['layout'] != 'full') : ?>
            <aside id="secondary" class="widget-area <?php echo esc_attr($page_layout['sidebar']); ?>"
                   role="complementary"
                   aria-label="<?php esc_attr_e('Post Sidebar', 'armania'); ?>">
